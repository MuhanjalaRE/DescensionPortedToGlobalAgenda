...
namespace game_data {
static ATgPlayerController* local_player_controller = NULL;
...
class Player : public GameActor {
   ...
   public:
    ...
    void Setup(Character* character) {
        ...
        team_id_ = game_data::local_player_controller->IsEnemy(character_);  // character->GetTeamNum();
...
void GetPlayers(void) {
    ...
    while (pawn != NULL) {
        if (validate::IsValid(pawn)) {
            //wcout << pawn->PlayerReplicationInfo->PlayerName.Data << endl;
...
FVector2D Project(FVector location) {
    FVector projection_3d = WorldToScreen(location);
    return {projection_3d.X, projection_3d.Y};
    // return {0, 0};
}

FVector2D Project_(FVector location) {
...
FVector WorldToScreen(FVector WorldLocation) {
    ...
    Delta = game_data::local_player_controller->Subtract_VectorVector(WorldLocation, CameraLocation);
    Transformed.X = game_data::local_player_controller->Dot_VectorVector(Delta, Y);
    Transformed.Y = game_data::local_player_controller->Dot_VectorVector(Delta, Z);
    Transformed.Z = game_data::local_player_controller->Dot_VectorVector(Delta, X);
    ...
    float FOVAngle = game_data::local_player_controller->FOVAngle;

    Out.X = ScreenCenterX + Transformed.X * (ScreenCenterX / game_data::local_player_controller->Tan(FOVAngle * CONST_Pi / 360.0f)) / Transformed.Z;
    Out.Y = ScreenCenterY + -Transformed.Y * (ScreenCenterX / game_data::local_player_controller->Tan(FOVAngle * CONST_Pi / 360.0f)) / Transformed.Z;
    Out.Z = 0;
    return Out;
}
...
void Tick(void) {
    ...
    if (use_muzzle_location) {
        ...
        muzzle_offset = game_data::my_player.character_->eventGetWeaponStartTraceLocation((ATgDevice*)game_data::my_player.character_->Weapon) - game_data::my_player.location_;
    }
            ...
            if (result) {
                ...
                if (aimbot_settings.use_triggerbot) {
                        ...
                        if (los) {
                            ...
                            if (abs(game_data::screen_center.X - projection.X) < width && abs(game_data::screen_center.Y - projection.Y) < height) {
                                if (game_data::my_player.weapon_ == game_data::Weapon::found) {
                                    ...
                                    game_data::local_player_controller->StartFire();
                    ...
                    if (!triggerbot_success) {
                        game_data::local_player_controller->StopFire();
                    };
                }
...
void GetWeapon(void) {
    ...
    ATgDevice* weapon = (ATgDevice*)my_player.character_->Weapon;
    if (weapon) {
        game_data::my_player.weapon_ = game_data::Weapon::found;

        bool projectile_weapon = true;  // weapon->IsA(ATrDevice_ThrowingKnives::StaticClass()) || weapon->IsA(ATrDevice_SN7::StaticClass()) || weapon->IsA(ATrDevice_SN7_MKD::StaticClass());
        ...
        if (!projectile_weapon /* && weapon->bInstantHit && !weapon->IsA(ATrDevice_ConstantFire::StaticClass())*/) {
            ...
            weapon_parameters->bullet_speed_ = weapon->m_FireMode.Data[0]->GetProjectileSpeed();
            weapon_parameters->inheritence_ = 0;

            /*
            ATgProjectile* p = (ATgProjectile*)weapon->Spawn(weapon->m_FireMode.Data[0]->GetProjectileClass(), weapon, FName(0), FVector({-999999, -999999, -999999}), FRotator(), nullptr, 0);
            if (p) {
                weapon_parameters->bullet_speed_ = p->Speed;
                cout << p->Speed << endl;
                weapon_parameters->inheritence_ = 0;  // p->m_fMaxProjInheritPct;
                abstraction::my_weapon_object.SetWeaponType(abstraction::WeaponObject::WeaponType::kProjectileLinear);
...
PROCESSEVENT_HOOK_FUNCTION(UEHookMain) {
    ...
    if (ue::frame_is_ready) {
        ...
        static UGameEngine* ugameengine = UObject::FindObject<UGameEngine>("GameEngine Transient.GameEngine");

        // ATrHUD_eventPostRenderFor_Parms* param = (ATrHUD_eventPostRenderFor_Parms*)p;
        game_data::local_player_controller = (ATgPlayerController*)ugameengine->GamePlayers.Data[0]->Actor;
        game_data::local_player_character = (Character*)game_data::local_player_controller->Pawn;

        if (game_data::local_player_character) {
            ATgDevice* weapon = (ATgDevice*)game_data::local_player_character->Weapon;
            if (weapon) {
        ...
        if (game_data::my_player.is_valid_ || true) {
            game_data::local_player_controller->eventGetPlayerViewPoint(&game_functions::CameraLocation, &game_functions::CameraRotation);
